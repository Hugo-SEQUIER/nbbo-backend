...
# HTTP endpoint to get latest trades (fallback)
@router.get("/trades/latest")
async def get_latest_trades():
    """Get the latest trade data for all supported coins"""
    return TradeResponse(
        success=True,
        data={coin: trade for coin, trade in latest_trades.items() if trade},
        timestamp=int(asyncio.get_event_loop().time() * 1000)
    )

# HTTP endpoint to get latest trade for a specific coin
@router.get("/trades/latest/{coin}")
async def get_latest_trade_for_coin(coin: str):
    """Get the latest trade data for a specific coin"""
    coin = coin.upper()
    if coin not in SUPPORTED_COINS:
        return {"success": False, "error": f"Unsupported coin: {coin}"}
    
    if not latest_trades.get(coin):
        return {"success": False, "error": f"No trade data available for {coin}"}
    
    return TradeResponse(
        success=True,
        data={coin: latest_trades[coin]},
        timestamp=int(asyncio.get_event_loop().time() * 1000)
    )

# HTTP endpoint to get the absolute latest trade across all coins
@router.get("/trades/latest-overall")
async def get_latest_trade_overall():
    """Get the most recent trade data across all supported coins"""
    if not latest_trade_overall:
        return {"success": False, "error": "No trade data available"}
    
    return TradeResponse(
        success=True,
        data={latest_trade_overall["coin"]: latest_trade_overall},
        timestamp=latest_trade_overall["timestamp"]
    )
